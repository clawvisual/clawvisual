...
- 支持按题材自动匹配视觉风格（儿童科教/科技报道/商业新闻），并保持 icon 与字体风格全局一致
- 当前生产主路径聚焦 `longform_digest` 模式
...
1. 输入 URL 或长文本
2. 执行主流水线：`skill_input_processor` -> `skill_content_planner` -> `skill_visual_prompt_planner` -> `skill_asset_generator` -> `skill_viral_optimizer`
3. 轮询异步任务状态直到完成
...
左侧 `Your chats` 区域右上角提供 `Clear all` 按钮，可一键清空全部会话。
...
首次运行时的预期行为：
- 接口会先快速返回 `202`，任务异步执行
- `fast` 模式会附加 `skill_quality_copy_polish`
- `full` 模式会在此基础上额外执行 `skill_quality_post_copy_loop` 与 `skill_quality_final_audit`
...
- API：
  - `POST /api/v1/convert`：启动异步转换流水线并返回 `job_id`
...
- 技能系统：`src/lib/skills` 当前保留长文主链核心模块：
  - `input-processor.ts`
  - `content-planner.ts`
  - `visual-prompt-planner.ts`
  - `asset-generator.ts`
  - `viral-optimizer.ts`
- 流水线注册：`src/lib/pipeline/registry.ts` 按内容模式定义：
...
- `src/lib/skills`：长文主链技能模块
- `src/lib/pipeline/registry.ts`：按内容模式和运行模式（`fast`/`full`）配置流水线
